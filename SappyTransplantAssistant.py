...
u""" Sappy Transplant Assistant ver 1.1 by ideal.exe
    ...
    与えられたソングテーブル内の曲が使用しているボイスセットを元のアドレス＋指定オフセットでアクセス出来るようにポインタを書き換えるツール
    ソングテーブルのアドレスは他のツールで調べてください．ソングテーブル以外のアドレスを入力するとどうなるかはわかりません．
"""
...
def voiceTransplanter(romData, songTableAddr, transplantOffs):
    u""" 指定したソングテーブル内の曲が使用しているボイスセットに元のアドレス + transplantOffsでアクセスできるようにポインタを調整する
