...
class BattleCoordinator():
    ...
    def dumpBattleData(self, addr):
        ...
        [field, x1, bgm, mode, background, count, area, x2, effect, objAddr] = \
            struct.unpack("BBBBBBBBLL", battleData)
        ...
        while self.romData[readAddr] != OBJECT_SEPARATOR:
            ...
            logger.debug("\t\t'objType':" + hex(objType) + "\t# " + \
                EXE6Dict.OBJECT_TYPE_LIST[objType])
    ...
    def importBattleData(self, battleDataSet):
        ...
        for d in battleDataSet:
            ...
            battleData = struct.pack("BBBBBBBBLL", field, x1, bgm, mode, \
                bg, count, area, x2, effect, objAddr+MEMORY_OFFSET)
            logger.debug("Raw Battle Data:\t" + str(battleData))
